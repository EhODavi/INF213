...
void MyHashMap<KEY,VALUE,HASH>::set(const  KEY &k, const VALUE &v) {
	...
	table[hash].push_back(make_pair(k,v));
	numElems++;
}
...
void MyHashMap<KEY,VALUE,HASH>::reHash(int newSize) {
	int hash;
	MyHashMap<KEY,VALUE,HASH> aux = *this;

	table.clear();
	table.resize(newSize);
	tableSize = newSize;

	for(int i = 0; i < aux.tableSize; i++) {
		for(typename MyList2<std::pair<KEY,VALUE> >::iterator it = aux.table[i].begin(); it != aux.table[i].end(); it++) { 
			hash = funcaoHash((*it).first) % tableSize;
			table[hash].push_back(make_pair((*it).first,(*it).second));
		}
	}
}
