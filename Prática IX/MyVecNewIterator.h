...
public:
	...
	iterator begin() {return data;} //Exercicio: e se tivermos uma lista constante, como itera-la para, por exemplo, imprimir os elementos? Basta pegar o begin, imprime o ponteiro derreferenciado e usar o ++ até chegar no end
	...
	const iterator begin() const {return data;} //Exercicio: e se tivermos uma lista constante, como itera-la para, por exemplo, imprimir os elementos?  Basta pegar o begin, imprime o ponteiro derreferenciado e usar o ++ até chegar no end
...
void MyVec<T>::push_back(const T&elem) {
	if(dataSize + 1 >= dataCapacity) { //preciso redimensionar?
		if(dataCapacity ==0) //Por que e' preciso testar isso? Porque 2 * 0 = 0
			resizeCapacity(1);
		else
...
void MyVec<T>::insert(const T&elem,int pos) {
	...
	if(dataSize + 1 >= dataCapacity) { //preciso redimensionar?
		if(dataCapacity ==0) //Exercicio: Por que e' preciso testar isso? Porque 2 * 0 = 0
			resizeCapacity(1);
		else
			resizeCapacity(dataCapacity*2);
	}	
...
MyVec<T> & MyVec<T>::operator=(const MyVec &other) {
	...
	destroy(); //Exercicio: por que precisamos disso? Para desalocar a memória do this
	dataCapacity = other.dataCapacity;
	dataSize = other.dataSize;
	//data = other.data; //por que nao podemos fazer isso? Para não dar double free
