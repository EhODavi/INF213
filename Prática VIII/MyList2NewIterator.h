...
public:
	...
	bool empty() {return begin() == iterator(NULL, this);}
	int size(); /*etapa 2: implemente esta funcao sem usar o membro de dados dataSize*/  // na STL List, a funcao size() calcula o tamanho da lista dinamicamente (exercicio: qual a ordem de complexidade?) O(n)
...
public:
	...
	//exercicio: faz sentido ter operatores <, > , <=, etc? Não pois os elementos da lista não estão na memória de forma contínua. Com isso, essas operações seriam ineficientes
	//           faz sentido termos operadores +, =+ (para "pularmos" n elementos em vez de 
	//                                                avancarmos de um em um) , etc? Não pois os elementos da lista não estão na memória de forma contínua. Com isso, essas operações seriam ineficientes

	//exercicio: armazenamos um ponteiro nesta classe. Deveriamos ter destrutor, constr. de copia, etc? Não pois o ponteiro sempre aponta para um Nodo de uma lista. A lista já implementa essas funções
...
MyList2<T> & MyList2<T>::operator=(const MyList2 &other) {
	...
	clear(); //Exercicio: por que precisamos disso? Para desalocar a memória do this
...
void MyList2<T>::insert(const T&elem, iterator whereIt) {
	...
	if(whereIt == end()) { //caso especial: insercao no final da lista... 
		push_back(elem);
	} else if (whereIt==begin()) { //caso especial: insercao no inicio da lista (push_front)
...
typename MyList2<T>::iterator MyList2<T>::erase(iterator elemIt) { //remove o elemento apontado por Elem
	...
	dataSize--;
	if(elem==dataFirst && elem==dataLast) { //exercicio: por que precisamos desse caso especial? Para quando só há um elemento na lista
