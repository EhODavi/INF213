...
void reverse(MyList2<T> &lista) {
  typename MyList2<T>::iterator begin = lista.begin();
  typename MyList2<T>::iterator end = lista.begin();
  typename MyList2<T>::iterator curr(NULL, NULL);
  int count = 0;
...
void testaDecrementoEnd() {
  ...
  //vai acessar o novo valor corretamente? Irá sim
  MyList2<int>::iterator it = l1.end();
  l1.insert(6,l1.end());
  it--;
...
int main() {
  ...
  if(tipoTeste=='N') { //testes com funcao reverse
    ...
    testaReverse();
  }  else if(tipoTeste=='D') { //testes com decremento do end()
