...
public:
    ...
    Conjunto<T> operator+(const Conjunto<T> &c) const;
    Conjunto<T> operator*(const Conjunto<T> &c) const;
    Conjunto<T> operator-(const Conjunto<T> &c) const;
private:
    T *elementos;
...
Conjunto<T> Conjunto<T>::operator+(const Conjunto<T> &c) const {
    Conjunto<T> C(tam_array + c.tam_array);

    for (int i = 0; i < num_elementos; i++) C.insere(elementos[i]);
    for (int i = 0; i < c.num_elementos; i++) C.insere(c.elementos[i]);
...
Conjunto<T> Conjunto<T>::operator*(const Conjunto<T> &c) const {
    ...
    if (tam_array < c.tam_array) novoTamanho = tam_array;
    else novoTamanho = c.tam_array;
    
    Conjunto<T> C(novoTamanho);

    for (int i = 0; i < num_elementos; i++) {
        if (c.pertence(elementos[i])) C.insere(elementos[i]);
    }
